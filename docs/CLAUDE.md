...
### initData 验证必须在后端做
前端拿到 window.Telegram.WebApp.initData 后，
每次请求都放在 Header 里传给后端验证，
前端不做任何身份判断逻辑。

### 前端禁止硬编码 API 地址
统一放在 frontend/config.js 的 CONFIG.API_BASE_URL，
本地和生产环境只改这一个文件。

### 前端 JS 全部走统一的 apiCall 函数
不得裸 fetch，统一封装错误处理和 Header 注入。

## 🟡 Railway 数据库本地连接
本地开发时 Railway 公网地址可能连接不稳定，
不要在本地花时间调试数据库连接问题。
直接部署到 Railway 用内网地址测试是更可靠的方式。