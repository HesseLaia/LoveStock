...
**验收标准：**
- [x] `backend/package.json` 存在且包含所有依赖
- [x] `backend/.env` 文件存在（包含占位符）
- [x] `backend/.env` 未被 git 追踪
- [x] 运行 `npm run dev` 不报错（即使 server.js 为空）
...
**验收标准：**
- [x] Railway MySQL 服务已创建
- [x] `backend/src/db/pool.js` 实现连接池
- [x] 使用本地 `.env` 公网地址可成功连接
- [x] 代码中无硬编码的数据库地址
- [x] 运行测试查询成功（如 `SELECT 1`）
...
**验收标准：**
- [x] `backend/init.sql` 文件存在
- [x] `users` 表已创建，包含所有字段
- [x] `answers` 表已创建，包含外键和唯一索引
- [x] `chart_data` 和 `answers_data` 字段类型为 LONGTEXT
- [x] 运行 `SHOW TABLES` 显示两张表
...
**验收标准：**
- [x] 运行 `npm run dev` 服务器启动成功
- [x] 访问 `http://localhost:3000/health` 返回 `{"status": "ok"}`
- [x] 控制台显示 "Environment variables loaded successfully"
- [x] 缺少必需环境变量时服务器拒绝启动
...
**验收标准：**
- [x] `validator.js` 导出 `validateTelegramInitData` 函数
- [x] 无效的 initData 返回 null
- [x] 过期的 initData（>24小时）返回 null
- [x] 有效的 initData 返回解析后的 user 对象
- [x] `authMiddleware` 验证失败返回 401
...
**验收标准：**
- [x] `getUserResult` 能正确读取用户结果，不存在返回 null
- [x] `saveUserResult` 能插入新记录
- [x] `saveUserResult` 能覆盖旧记录（使用 ON DUPLICATE KEY UPDATE）
- [x] 所有 JSON 数据正确序列化和反序列化
- [x] 数据库连接在 finally 块中正确释放
- [x] 手动插入测试数据后能正确读取
...
**基础分值计算（8.1）：**
- [x] 输入 8 个 A 返回正确的总分值
- [x] 输入 8 个 D 返回正确的总分值（Q5-D 需随机）
- [x] Q5-D 多次计算返回不同结果（验证随机性）
- [x] 分值范围在 -50 到 +75 之间

**股票类型判断（8.2）：**
- [x] 输入 `['A','A','A','A','A','B','B','B']` 返回 "Blue Chip"
- [x] 输入 `['A','A','A','A','A','A','A','A']` 返回 "Concept Stock"
- [x] 输入 `['A','A','A','A','A','C','C','C']` 返回 "Defensive Stock"
- [x] 未匹配的组合返回 "Growth Stock"

**最终价格计算（8.3）：**
- [x] baseScore = -50 时价格接近 $1
- [x] baseScore = +75 时价格接近 $9999
- [x] Blue Chip 价格比 Growth Stock 高约 20%
- [x] 价格范围严格在 $1.00 - $9999.99
- [x] 多次调用同一输入返回不同结果（验证随机性）

**涨跌幅和等级（8.4）：**
- [x] 激进答案（高分）返回接近 +99.9% 的涨跌幅
- [x] 保守答案（低分）返回接近 -20.0% 的涨跌幅
- [x] 涨跌幅保留一位小数
- [x] 价格 $8000+ 返回 A+
- [x] 价格 $500- 返回 C-
- [x] 边界价格（如 $4500）返回正确等级

**特殊标签（8.5）：**
- [x] Q1-D 返回 "RARE FIND"
- [x] Q2-A 返回 "HIGH LIQUIDITY"
- [x] Q2-D 返回 "LONG-TERM VALUE"
- [x] Q5-D 返回 "PRE-IPO ASSET"
- [x] 无匹配时返回 null
- [x] 多个匹配时只返回第一个

**K 线图数据（8.6）：**
- [x] Blue Chip 生成平稳上升曲线（波动小）
- [x] Concept Stock 生成大起大落曲线（波动大）
- [x] Defensive Stock 生成接近水平曲线（波动极小）
- [x] 所有数据点在 0-52 范围内
- [x] 返回数组长度为 30
- [x] 多次调用返回不同结果

**完整估值函数（8.7）：**
- [x] `calculateValuation(['A','A','A','A','A','A','A','A'], 'alex')` 返回完整对象
- [x] 返回对象包含所有必需字段：ticker, final_price, change_percent, stock_type, grade, special_tag, chart_data
- [x] ticker 正确生成（如 "$ALEX"）
- [x] 无 username 时 ticker 为 "$USER"
- [x] 编写单元测试覆盖核心逻辑
...
**验收标准：**
- [x] 正常调用返回 AI 生成的点评（25 词以内）
- [x] 超时时返回兜底文案
- [x] API 错误时返回兜底文案
- [x] 不重试，直接降级
- [x] 日志记录错误信息
- [x] 模型 ID 使用 `google/gemini-2.0-flash-001` 或 `google/gemini-2.5-flash`
...
**验收标准：**
- [x] 模型 ID 在 OpenRouter 网站上存在且 Active
- [x] 测试调用成功返回内容
- [x] 响应时间在 5 秒以内
- [x] 兜底逻辑正常工作
...
**验收标准：**
- [x] 无 initData 返回 401
- [x] 无效 initData 返回 401
- [x] 首次用户返回 `has_result: false`
- [x] 有历史结果用户返回完整结果对象
- [x] 响应格式符合 API 规范
- [x] 使用 Postman 测试成功
...
**验收标准：**
- [x] 答案格式错误返回 400 + `INVALID_ANSWERS`
- [x] 答案少于 8 个返回 400
- [x] 答案包含非法选项（如 "E"）返回 400
- [x] 正确答案返回完整结果对象
- [x] 结果正确保存到数据库（覆盖旧记录）
- [x] AI 点评生成失败时使用兜底文案
- [x] 响应时间在 10 秒以内
...
**验收标准：**
- [x] `frontend/index.html` 不包含 `<style>` 和 `<script>` 标签
- [x] `frontend/styles.css` 包含所有样式
- [x] `frontend/app.js` 包含所有逻辑
- [x] `frontend/config.js` 包含 API_BASE_URL
- [x] 本地打开 `index.html` 样式和功能正常
...
**验收标准：**
- [x] 所有逻辑封装在 `LoveStock` 对象内
- [x] 无全局变量污染（除了 LoveStock）
- [x] 状态集中管理在 `state` 对象
- [x] 页面切换通过 `navigateTo` 方法
- [x] 代码结构清晰，易于维护
...
- [x] 网络错误显示友好提示
- [x] 401 错误显示认证失败页
- [x] 所有 fetch 请求带 `X-Telegram-Init-Data` header
...
**验收标准：**
- [x] Railway 服务部署成功
- [x] 访问 `https://<domain>/health` 返回 200
- [x] `/api/init` 端点可访问
- [x] `/api/valuation` 端点可访问
- [x] 后端日志无错误
- [x] 数据库连接成功（使用内网地址）
...
**验收标准：**
- [x] Vercel 部署成功
- [x] 访问 `https://<domain>` 显示欢迎页
- [x] 页面样式正常
- [x] 可以正常答题
- [x] 能正常请求后端 API（无 CORS 错误）
- [x] 移动端显示正常
...
- [x] Bot 创建成功
- [x] 发送 `/start` 返回欢迎消息
- [x] 点击按钮打开 Vercel 部署的 Mini App
- [x] Mini App 能正确获取 Telegram 用户信息
- [x] Bot 部署到 Railway 且运行稳定
...
|------|--------|--------|--------|--------|
| 阶段一：后端基础 | 7 | 7 | 0 | 0 |
| 阶段二：估值算法 | 1 (合并 Task 8-13) | 1 | 0 | 0 |
| 阶段三：OpenRouter | 2 | 2 | 0 | 0 |
| 阶段四：API 端点 | 2 | 2 | 0 | 0 |
| 阶段五：前端开发 | 4 | 4 | 0 | 0 |
| 阶段六：部署集成 | 3 | 3 | 0 | 0 |
| 阶段七：测试优化 | 1 | 1 | 0 | 0 |
...
以下是已识别的待优化项，计划在下一版本迭代中解决：

### UI/UX 改进
- [ ] **答题页卡片抖动修复**：选项点击后卡片有轻微抖动，需要优化动画时序
- [ ] **Share Tech Mono 字体渲染问题**：部分设备上等宽字体渲染不一致，考虑使用 woff2 格式或本地托管
- [ ] **UI 整体视觉打磨**：
  - [ ] 结果页颜色对比度调整（WCAG AA 标准）
  - [ ] 按钮点击反馈优化
  - [ ] 加载动画更流畅
  - [ ] 移动端适配优化（小屏幕设备）
...
- [ ] **更多股票类型和题目**：
  - [ ] 增加第 9-12 题（可选）
  - [ ] 新增股票类型：Penny Stock（低价股）、IPO Stock（新股）
  - [ ] 更丰富的特殊标签组合
- [ ] **分享功能**：
  - [ ] 生成结果卡片图片（Canvas 或 SVG 转图片）
  - [ ] 分享到 Telegram 群组/频道
  - [ ] 自定义分享文案
- [ ] **排行榜**：
  - [ ] 全局估值排行榜（Top 100）
...
### 性能优化
- [ ] **K 线图渲染优化**：使用 Canvas 替代 SVG，提升大数据量渲染性能
- [ ] **OpenRouter 调用优化**：
  - [ ] 批量生成点评（后台任务）
  - [ ] 缓存常见结果组合的点评
- [ ] **数据库查询优化**：
